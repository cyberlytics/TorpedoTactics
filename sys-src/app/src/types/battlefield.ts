...
export function createGrid(gridSize: number,amountShips: number){
    let Grid : cellState[][] = Array(gridSize).fill(null).map(() => Array(gridSize).fill(cellState.empty));
    console.log(Grid);
    let count : number = 0;
...
export function createEmptyGrid(gridSize: number){
     return Array(gridSize).fill(null).map(() => Array(gridSize).fill(cellState.empty));
}
